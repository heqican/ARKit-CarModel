...
- (void)rearviewMirrorClose:(BOOL)active{
    if (active) {//合上后视镜
        [self closeRearviewMirrorModel:@"Object005" Duration:1.0];
        [self closeRearviewMirrorModel:@"black_m_18" Duration:1.0];
    }else{//打开后视镜
        [self openRearviewMirrorModel:@"Object005" Duration:1.0];
        [self openRearviewMirrorModel:@"black_m_18" Duration:1.0];
    }
}

//合上后视镜 black_m_18 Object005
- (void)closeRearviewMirrorModel:(NSString *)modelName Duration:(NSTimeInterval)duration{
    for (SCNNode *partsNode in self.carModelNode.childNodes) {
        if ([partsNode.name isEqualToString:modelName]) {
            
//            partsNode.pivot = SCNMatrix4MakeTranslation(100, 0, 0);
            
            //创建自转动画
            CABasicAnimation *animation = [CABasicAnimation animationWithKeyPath:@"rotation"];//执行的是旋转
            animation.duration = duration;
            animation.toValue = [NSValue valueWithSCNVector4:SCNVector4Make(0,1,0, M_PI/2)];//转90度
            animation.repeatCount = 1;
            [partsNode addAnimation:animation forKey:@"rearviewMirror rotation"];
            [partsNode runAction:[SCNAction repeatAction:[SCNAction rotateByX:0 y:2 z:0 duration:duration] count:1]];//后视镜绕Y轴旋转
        }
    }
}

//打开后视镜
- (void)openRearviewMirrorModel:(NSString *)modelName Duration:(NSTimeInterval)duration{
    for (SCNNode *partsNode in self.carModelNode.childNodes) {
        if ([partsNode.name isEqualToString:modelName]) {
            //创建自转动画
            CABasicAnimation *animation = [CABasicAnimation animationWithKeyPath:@"rotation"];
